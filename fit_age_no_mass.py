...
def get_Gaia_magnitudes(mass, metallicity, age):
    ...
    return float(Gaia_G_EDR3), float(Gaia_BP_EDR3), float(Gaia_RP_EDR3)
...
def fit_age(Fe_H, Fe_H_err, G, G_err, Bp, Bp_err, Rp, Rp_err):
    ...
    guess, mass_err = initial_guess(Fe_H, G, Bp, Rp, G_err, Bp_err, Rp_err)
    ...
    fit = least_squares(residuals_Gaia, guess, args=(G, Bp, Rp, G_err, Bp_err, Rp_err, guess[0], mass_err, Fe_H, Fe_H_err), bounds=bounds)
...
sample = fits.open('gaia_giants.fits')
...
Abp = Rbp/(Rbp-Rrp) * targets['ebpminrp_gspphot']
Arp = Rrp/(Rbp-Rrp) * targets['ebpminrp_gspphot']

# Extract errors in extinction
Ag_err = np.max([np.abs(targets['ag_gspphot_lower']-Ag), np.abs(targets['ag_gspphot_upper']-Ag)], axis=0)
Abp_err = np.max([np.abs(targets['ebpminrp_gspphot_lower']*Rbp/(Rbp-Rrp) - Abp), np.abs(targets['ebpminrp_gspphot_upper']*Rbp/(Rbp-Rrp) - Abp)], axis=0)
Arp_err = np.max([np.abs(targets['ebpminrp_gspphot_lower']*Rrp/(Rbp-Rrp) - Arp), np.abs(targets['ebpminrp_gspphot_upper']*Rrp/(Rbp-Rrp) - Arp)], axis=0)
...
for i in range(len(Fe_H)):
    ...
    # run least square fit 
    fitted, errs = fit_age(Fe_H[i], Fe_H_err[i], mg[i], mg_err[i], mbp[i], mbp_err[i], mrp[i], mrp_err[i])
...
targets_table.write('gaia_giants_fitted.fits', format='fits', overwrite=True)
print('Fitted parameters saved to gaia_giants_fitted.fits')
# Make graph of age vs metallcity
#fig = plt.figure(figsize=(12, 6))
#ax1 = fig.add_subplot(1,2,1)
#ax1.hist2d(fitted[2], fitted[1], bins=[50, 50], cmap='Blues', cmin=1)
#ax1.set_xlabel('Fitted Age (Gyr)')
#ax1.set_ylabel('Fitted [Fe/H] (dex)')
#ax1.set_title('Fitted Age vs Fitted [Fe/H]')

#ax2 = fig.add_subplot(1,2,2)
#ax2.hist2d(fitted[2], targets['mh_gspphot'], bins=[50, 50], cmap='Blues', cmin=1)
#ax1.set_xlabel('Fitted Age (Gyr)')
#ax1.set_ylabel('Gaia [Fe/H] (dex)')
#ax1.set_title('Fitted Age vs Gaia [Fe/H]')
